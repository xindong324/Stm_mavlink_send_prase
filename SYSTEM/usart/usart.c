...
void handleMessage(mavlink_message_t msg)
{
	...
	mavlink_vicon_position_estimate_t vicon_position;
	...
	{
		mavlink_msg_vicon_position_estimate_decode(&msg, &vicon_position);
		printf("vicon rcv\n");
	}
}
...
void USART2_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if (mavlink_parse_char(MAVLINK_COMM_0, Res, &msg, &status))
		{
			printf("msg rcv\n");
			handleMessage(msg);
		} 		 
